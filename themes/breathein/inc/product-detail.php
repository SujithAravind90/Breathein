...
function breathein_product_buy_now_redirect(string $url): string
{
    ...
    if (function_exists('wc_get_cart_url')) {
        $compare_page = get_page_by_path('compare', OBJECT, 'page');
        $compare_url = $compare_page instanceof WP_Post
            ? get_permalink($compare_page)
            : home_url('/compare/');
        $referer = wp_get_referer();

        $referer_path = $referer ? untrailingslashit((string) wp_parse_url($referer, PHP_URL_PATH)) : '';
        $compare_path = untrailingslashit((string) wp_parse_url($compare_url, PHP_URL_PATH));

        if (is_page('compare') || ($referer_path !== '' && $referer_path === $compare_path)) {
