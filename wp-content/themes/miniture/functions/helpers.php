...
if ( ! function_exists( 'nova_get_post_meta' ) ) {
    function nova_get_post_meta( $object_id, $sub_key = '', $meta_key = '', $single = true ) {
        ...
        if (empty($meta_key)) {
            $meta_key = '_miniture_post_options';
        }
        ...
        if(has_post_thumbnail()){ ?>
            <div class="post-thumbnail">
							...
							if( 'video' == get_post_format() && NOVA_RWMB_IS_ACTIVE  ){
									$popup_video_link = rwmb_meta('miniture_post_video_url');
									$id = rand();
...
//==============================================================================
if ( ! function_exists( 'miniture_excerpt' ) ) {

	function miniture_excerpt( $length = 30 ) {
		global $post;
...
if ( ! function_exists( 'nova_post_time_ago' ) ) {
	function nova_post_time_ago( $type = 'post' ) {
	    ...
	    return human_time_diff($d('U'), current_time('timestamp')) . " " . esc_html__('ago','miniture');
