...
function nova_breadcrumbs( $args = '' ) {
	$args = wp_parse_args( $args, array(
		...
		'show_on_front'     => false,
		'labels'            => array(
			'home'      => esc_html__( 'Home', 'miniture' ),
			'archive'   => esc_html__( 'Archives', 'miniture' ),
			'blog'      => esc_html__( 'Blog', 'miniture' ),
			'search'    => esc_html__( 'Search results for', 'miniture' ),
			'not_found' => esc_html__( 'Not Found', 'miniture' ),
			'author'    => esc_html__( 'Author Archives:', 'miniture' ),
			'day'       => esc_html__( 'Daily Archives:', 'miniture' ),
			'month'     => esc_html__( 'Monthly Archives:', 'miniture' ),
			'year'      => esc_html__( 'Yearly Archives:', 'miniture' ),
		),
	) );
	...
	elseif ( is_day() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'miniture' ), $args['labels']['day'], get_the_date() )
		);
	} // Month archive
	elseif ( is_month() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'miniture' ), $args['labels']['month'], get_the_date( 'F Y' ) )
		);
	} // Year archive
	elseif ( is_year() ) {
		$items[] = sprintf(
			$item_text_tpl,
			sprintf( esc_html__( '%s %s', 'miniture' ), $args['labels']['year'], get_the_date( 'Y' ) )
		);
	} // Archive
	else {
		$items[] = sprintf(
